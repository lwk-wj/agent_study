...
// 将向量数据库转为检索器 Retriever，并指定检索最相似的模块 （k: 分块数）
...
for (const q of question) {
    ...
    // RAG 步骤 C：将检索到的文档分块（Context）拼接并格式化，构建上下文背景 (构建prompt)
    const context = retrievedDocs.map((doc, i) => `[片段${i + 1}]\n${doc.pageContent}`).join("\n\n------\n\n")

    // RAG 步骤 D：将上下文与问题组合，组装成最终的提示词（Prompt）
    ...
    // RAG 步骤 E：将完整的 Prompt 输入大语言模型，并打印 AI 生成的优质回答
    console.log("\n【AI 回答】");
